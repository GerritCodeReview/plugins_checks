...
UUIDs are [opaque URIs](https://tools.ietf.org/html/rfc3986#section-1.2.3) of
the form `SCHEME ':' ID`. The scheme should not be a normal URI scheme like
`http`, but instead a short string associated with the external system that
created the checker, for example `jenkins`. Because the URIs are opaque, ID part
may not start with `/`, but any other URL-encoded string is allowed. The server
normalizes the UUID before storing by converting the scheme to lowercase and
unencoding any unnecessarily %-encoded characters.

When passing a UUID as part of the URL, callers may use the exact form of the
string returned in the [CheckerInfo](#checker-info). For example, if the
CheckerInfo contains:

```
"uuid": "test:my%20checker"
```

then the checker is accessed as:

```
GET /plugins/checks/checkers/test:my%20checker
```

In particular, the `%20` must not be double-encoded as `%2520`. Any other form
that normalizes to the same UUID is valid, for example `TEST:my%20ch%65cker`.
...
| --------------- | -------- | ----------- |
| `uuid`          |          | The [UUID](#checker-id) of the checker.<br />The value in this field is already URL-safe and can be passed verbatim in the URL of REST calls.
