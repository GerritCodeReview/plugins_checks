...
updated. Properties that are not set in the input (that have `null` as value)
are not touched.
...
* `query`: Can be unset by setting an empty string ("") for it.
...
By default, checkers are considered relevant for all changes in their configured
`repository`. If the `query` field is specified in the checker config, then the
checker is only relevant for changes matching that query. The query field is
...
`added, assignee, author, branch, committer, deleted, delta, destination,
dir, directory, ext, extension, f, file, footer, hashtag, intopic, label,
onlyextensions, onlyexts, ownerin, path, r, ref, reviewer, reviewerin, size,
status, submittable, topic, unresolved`
