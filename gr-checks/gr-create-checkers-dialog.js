(function() {
  ...
  const CREATE_CHECKER_URL = '/plugins/checkers/checkers/';
  const SCHEME_PATTERN = '/^[a-zA-Z0-9\-\_\.]*$/';
  ...
  Polymer({
    ...
    */
    ...
    _editChecker(checker) {
      const url = "/plugins/checkers/checkers/" + checker.uuid;
      return this.$.restAPI._send({
        method: 'POST',
        url,
        body: checker
    ...
    _getRepoSuggestions(filter) {
      ...
      return this.$.restAPI.getRepos(filter, REPOS_PER_PAGE).then(
        (repos) => {
          return repos.map (
