      ...
      getChecks(change._number, revisionNumber).then(checks => {
        ...
        if (checks && checks.length) {
          ...
          checks.forEach(check => {
            check.submit_impact = JSON.parse('{"required":false,"message":null,"overrides":[{"reason":"testing","overriden_on":"2020-04-02 17:44:16.0","overrider":{"id":1000000,"name":null,"displayname":null,"email":null,"username":null}}]}');
          };
          if (!this._checks) {
            this._checks = checks;
          } else {
