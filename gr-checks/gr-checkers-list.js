...
(function() {
  ...
  Polymer({
    ...
    _showCheckers(_checkers, _filter) {
      ...
      this._visibleCheckers = this._filteredCheckers.slice(this._startingIndex, this._startingIndex + this._checkersPerPage);
    },
    ...
    _computeShowNextButton(_startingIndex, _filteredCheckers) {
      ...
      if (_startingIndex + this._checkersPerPage < _filteredCheckers.length) {
        return true;
      }
      return false;
    },
    ...
    _computeShowPrevButton(_startingIndex, _filteredCheckers) {
      ...
      if (_startingIndex >= this._checkersPerPage) {
        return true;
      }
      return false;
    },
    ...
    _handleNextClicked() {
      if (this._startingIndex + this._checkersPerPage < this._filteredCheckers.length) {
        this._startingIndex += this._checkersPerPage;
      }
      this._visibleCheckers = this._filteredCheckers.slice(this._startingIndex, this._startingIndex + this._checkersPerPage);
    },

    _handlePrevClicked() {
      if (this._startingIndex >= this._checkersPerPage) {
        this._startingIndex -= this._checkersPerPage;
      }
      this._visibleCheckers = this._filteredCheckers.slice(this._startingIndex, this._startingIndex + this._checkersPerPage);
    },
    ...
    _getCheckers() {
      ...
      return this.getCheckers()
        .then(checkers => {
          ...
          this._visibleCheckers = checkers.slice(0, this._checkersPerPage);
          this._startingIndex = 0;
          this._loading = false;
        });
    },
