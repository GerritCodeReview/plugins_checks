...
import './gr-checkers-lists';

const basicFixture = fixtureFromElement('gr-checkers-list');
...
suite('gr-checkers-list tests', () => {
  ...
  setup(done => {
    ...
    element.$.listOverlay.open = () => Promise.resolve(() => {});
    flush(done);
  });
  ...
  test('renders checker list table headings', () => {
    ...
    const headings = checkersList.firstElementChild.firstElementChild.children;
  ...
  test('create checker button renders', () => {
    ...
    const button = element.shadowRoot
      .querySelector('#createNewContainer')
  ...
  suite('with checkers', () => {
    ...
    test('renders correct number of checkers', () => {
      const checkers = element.shadowRoot
        .querySelector('table > tbody:nth-child(2)')
        .querySelectorAll('tr');
    ...
    test('renders correct checkers', () => {
      ...
      for (let i = 0; i < checkers.length; i++) {
        ...
        const checkerRequired =
          CHECKERS[i].blocking && CHECKERS[i].blocking.length > 0
            ? 'YES'
            : 'NO';
