...
public class CheckerRefMigrationIT extends AbstractCheckersTest {
  ...
  public void migrateLegacyMetaRef() throws Exception {
    ...
    assertThat(checkerOperations.checkersOf(allProjects, "refs/meta/checkers/"))
        .containsExactly(checkerWithBadRef);
    assertThat(checkerOperations.checkersOf(allProjects)).isEmpty();

    checkerRefMigration.migrate();

    assertThat(checkerOperations.checkersOf(allProjects)).containsExactly(checkerWithBadRef);
    assertThat(checkerOperations.checkersOf(allProjects, "refs/meta/checkers/")).isEmpty();
  }
}
