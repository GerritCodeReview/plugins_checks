...
public class ListPendingChecksIT extends AbstractCheckersTest {
  ...
  public void cannotListPendingChecksForMalformedCheckerUuid() throws Exception {
    assertInvalidQuery("checker:notauuid", "invalid checker UUID: notauuid");
  }
  ...
  @Test
  public void listPendingChecksForSpecifiedStateLowerCase() throws Exception {
    CheckerUuid checkerUuid = checkerOperations.newChecker().repository(project).create();
    assertThat(
            queryPendingChecks(
                buildQueryString(checkerUuid)
                    + " state:"
                    + CheckState.NOT_STARTED.name().toLowerCase(Locale.US)))
        .hasSize(1);
  }
