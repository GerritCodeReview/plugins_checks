...
public class PendingChecks extends AbstractEndpoint {
  ...
  public List<PendingChecksInfo> list() throws RestApiException, IOException {
    try {
  ...
  private URI buildRequestUrl() throws URISyntaxException {
    if (!queries.isEmpty()) {
      ...
      for (Map.Entry<String, String> query : queries.entrySet()) {
        ...
        queryString += String.format("%s%s:%s", connector, query.getKey(), query.getValue());
      }
      uriBuilder.setParameter("query", queryString);
    }
