  ...
  public String getETag(Project.NameKey projectName, Change.Id changeId) {
    try {
      ...
      logger.atWarning().withCause(e).log(
          "Failed to compute ETag for checks of change %s in project %s", changeId, projectName);
      return ""; // The change ETag will be invalidated if the checks can be accessed the next time.
    }
  }
}
