...
import com.google.gerrit.plugins.checks.Checks.GetChecksOptions;
...
public class ListChecks implements RestReadView<RevisionResource> {
  ...
  public ImmutableList<CheckInfo> apply(RevisionResource resource)
    ...
    GetChecksOptions getChecksOptions = GetChecksOptions.builder().setBackfillChecks(true).build();
    ImmutableList<Check> allChecks =
        checks.getChecks(resource.getProject(), resource.getPatchSet().getId(), getChecksOptions);
