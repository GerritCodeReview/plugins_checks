...
public class QueryPendingChecks implements RestReadView<TopLevelResource> {
  ...
  public Response<List<PendingChecksInfo>> apply(TopLevelResource resource)
    ...
    Predicate<Check> predicate = query;
    ...
    if (countPredicates(predicate, CheckerPredicate.class) == 1) {
      ...
      pendingChecks = getPendingChecksOfChecker(checker.get(), query, changes);
    } else {
      // Scheme query
      String scheme =
          getSchemeFromQuery(predicate)
              .orElseThrow(
                  () ->
                      new IllegalStateException(
                          String.format("no checker scheme predicate found: %s", predicate)));
   ...
   */
  private static <CheckPredicate> int countPredicates(
      Predicate<Check> predicate, Class<CheckPredicate> predicateClass) {
