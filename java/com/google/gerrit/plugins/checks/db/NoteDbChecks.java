...
class NoteDbChecks implements Checks {
  ...
  public CombinedCheckState getCombinedCheckState(NameKey projectName, Id patchSetId)
    ...
    for (Map.Entry<String, Checker> entry : allCheckersOfProject.entrySet()) {
      ...
      if (!checker.isCheckerRelevant(changeData, queryBuilder)) {
        // Skips checkers which are not relevant with this change.
        continue;
      }

      if (!checks.containsKey(checkerUuid)) {
        throw new IllegalStateException(
            String.format(
                "no check about relevant checker %s for change %s, patch set %s",
                checkerUuid, changeData.getId().id, patchSetId.get()));
      }
