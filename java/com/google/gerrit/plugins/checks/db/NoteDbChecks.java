...
class NoteDbChecks implements Checks {
  ...
  public ImmutableList<Check> getChecks(Project.NameKey projectName, PatchSet.Id psId)
    ...
    addRelevantCheckers(checkers, result, cd, ps);

    // TODO(dborowitz): Sort? By updated? By UUID? By status?
    return result.build();
  }

  private void addRelevantCheckers(
      Map<CheckerUuid, Checker> checkers,
      ImmutableList.Builder<Check> result,
      ChangeData cd,
      PatchSet ps)
      throws OrmException {
    if (checkers.isEmpty()) {
    ...
    if (!ps.getId().equals(cd.change().currentPatchSetId())) {
      // The query system can only match against the current patch set; it doesn't make sense to
      // include relevant checkers for old patch sets.
      return;
    }
